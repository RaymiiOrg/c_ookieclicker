...
CookieNumber ItemStoreShelfView::getPriceOfItem(const CookieNumber &itemAmountInInventory, const Item &item) const {
    return ItemStore::getPriceOf(item, itemAmountInInventory, itemQuantity);
}

void ItemStoreShelfView::renderNotEnoughMoneyButDoHaveItemsInInventoryOutput(const Item &item,
...
void ItemStoreShelfView::handleInput(const std::string &input)
{
    if (wallet && itemstore && inventory && msg) {
        if (isInputRelevant(input)) {
            for (const Item &item : itemstore->getStoreInventory()) {
...
void ItemStoreShelfView::buyItem(const CookieNumber &amountToBuy, const Item &item)
{
...
bool ItemStoreShelfView::canPayForItem(const Item &item)
{
...
void ItemStoreShelfView::renderCanBuy(const Item &item, const CookieNumber &itemAmountInInventory, const CookieNumber & price) const {
